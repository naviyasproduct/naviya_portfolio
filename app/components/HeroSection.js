...
const Hyperspeed = ({
  ...
  useEffect(() => {
    ...
    const options = effectOptions || {
      ...
      colors: isLightMode ? {
        roadColor: 0xf5f5f5,        // Very light gray/white road
        islandColor: 0xd8d8d8,      // Light gray island
        background: 0xa0a0a0,        // Medium gray background (darker for contrast)
        shoulderLines: 0x505050,     // Dark gray shoulder lines
        brokenLines: 0x606060,       // Dark gray broken lines
        leftCars: [0xff1493, 0x9370db, 0xba55d3],   // Deep pink, purple
        rightCars: [0x4682b4, 0x1e90ff, 0x00bfff],  // Steel blue, dodger blue, deep sky blue
        sticks: 0x4169e1             // Royal blue
      } : {
        roadColor: 0x080808,
    ...
    (function () {
      try {
        const container = document.getElementById('lights');
        if (!container) {
          console.error('HeroSection: lights container not found');
          return;
        }
        ...
        myApp.loadAssets().then(myApp.init).catch(err => {
          console.error('HeroSection: Failed to initialize', err);
        });
      } catch (err) {
